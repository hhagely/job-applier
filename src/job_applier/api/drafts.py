...
def get_draft(
    ...
    return _draft_out(job.id, include_markdown=include_markdown)
...
def render_draft(request: Request, job: JobPosting = Depends(require_job)):
...
def download_draft_resume(job: JobPosting = Depends(require_job)):
    path = drafts.pdf_path(job.id, "resume")
    if not path.exists():
...
def download_draft_cover_letter(job: JobPosting = Depends(require_job)):
    path = drafts.pdf_path(job.id, "cover_letter")
    if not path.exists():
