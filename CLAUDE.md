...
- **Desktop app** (Electron) is being built on the long-lived `desktop-app` branch, one PR per phase; specs live in [plans/desktop-app/](plans/desktop-app/). `make electron` runs the dev shell (Phase 6). The desktop **redesign** (Phase 8) source is in Open Design (Windows), project id `b0919236-9bed-4b67-bcad-e57c0d35b867`; reference copies are in [plans/desktop-app/redesign/](plans/desktop-app/redesign/) — from WSL, re-pull via `"/mnt/c/Users/Herb/AppData/Roaming/Open Design/namespaces/release-stable-win/data/projects/b0919236-9bed-4b67-bcad-e57c0d35b867"` (see `redesign/HANDOFF.md`).
